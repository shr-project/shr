...
static void get_phonebook_info_callback(GError * error, GHashTable * info,
					gpointer userdata);
int calculate_free_entry_index(int min, int max, GPtrArray * entries,
			       struct ContactEditViewData *data);
static void retrieve_callback(GError * error, GPtrArray * messages,
			      gpointer userdata);

static void frame_edit_show(struct ContactEditViewData *data);
static void frame_edit_hide(struct ContactEditViewData *data);
static void frame_edit_save_clicked(struct ContactEditViewData *data,
				    Evas_Object * obj, void *event_info);
static void frame_edit_close_clicked(struct ContactEditViewData *data,
				     Evas_Object * obj, void *event_info);
...
static void frame_edit_save_callback(GError * error,
				     struct ContactEditViewData *data);
...
static void frame_close_yes_clicked(struct ContactEditViewData *data,
				    Evas_Object * obj, void *event_info);
static void frame_close_no_clicked(struct ContactEditViewData *data,
				   Evas_Object * obj, void *event_info);
...
struct ContactEditViewData *
contact_edit_view_show(struct Window *win, GHashTable * options)
{
	struct ContactEditViewData *data =
		g_slice_alloc0(sizeof(struct ContactEditViewData));
	data->win = win;
	data->id = -1;
	data->name = strdup("");
	data->number = strdup("");
	...
	*(data->data_pointer) = data;	// used for async dbus calls
	...
	if (options != NULL) {
		if (g_hash_table_lookup(options, "name")) {
			data->name =
				strdup(g_hash_table_lookup(options, "name"));
		...
		if (g_hash_table_lookup(options, "number")) {
			data->number =
				strdup(g_hash_table_lookup(options, "number"));
		}

		data->callback =
			g_hash_table_lookup(options, "change_callback");
		data->callback_data =
			g_hash_table_lookup(options, "change_callback_data");
	}

	if (data->id == -1) {
		ogsmd_sim_retrieve_phonebook("contacts", retrieve_callback,
...
void
contact_edit_view_hide(struct ContactEditViewData *data)
{
...
static void
get_phonebook_info_callback(GError * error, GHashTable * info,
			    gpointer userdata)
{
	struct ContactEditViewData **data =
		(struct ContactEditViewData **) userdata;
	if (*data == NULL) {
		free(data);
	}
	else {
		int min =
			g_value_get_int(g_hash_table_lookup(info, "min_index"));
		int max =
			g_value_get_int(g_hash_table_lookup(info, "max_index"));
		(*data)->free_entry_index =
			calculate_free_entry_index(min, max, (*data)->contacts,
						   *data);
...
void
add_entry_index(gpointer _entry, gpointer _indizes)
{
...
int
calculate_free_entry_index(int min, int max, GPtrArray * entries,
			   struct ContactEditViewData *data)
{
...
static void
retrieve_callback(GError * error, GPtrArray * contacts, gpointer userdata)
{
	struct ContactEditViewData **data =
		(struct ContactEditViewData **) userdata;
	if (*data == NULL) {
		...
		ogsmd_sim_get_phonebook_info("contacts",
					     get_phonebook_info_callback, data);
	}
}
...
static void
frame_edit_show(struct ContactEditViewData *data)
{
...
static void
frame_edit_hide(struct ContactEditViewData *data)
{
	struct Window *win = data->win;
...
static void
frame_edit_save_clicked(struct ContactEditViewData *data, Evas_Object * obj,
			void *event_info)
{
	char *temp_name = frame_edit_name_get(data);
	char *temp_number = frame_edit_number_get(data);
	if (!strlen(temp_name) || !strlen(temp_number)
	    || !string_is_number(temp_number)) {
		free(temp_name);
		free(temp_number);
		return;
	}
	...
	window_frame_show(data->win, data, frame_loading_show,
			  frame_loading_hide);
...
static void
frame_edit_save_callback(GError * error, struct ContactEditViewData *data)
{
...
static void
frame_edit_close_clicked(struct ContactEditViewData *data, Evas_Object * obj,
			 void *event_info)
{
...
static char *
frame_edit_name_get(struct ContactEditViewData *data)
{
...
static char *
frame_edit_number_get(struct ContactEditViewData *data)
{
	char *number =
		g_strstrip(strdup(elm_entry_entry_get(data->entry_number)));
...
static void
frame_close_show(struct ContactEditViewData *data)
{
	struct Window *win = data->win;

	window_layout_set(win, DIALOG_FILE, "close");

	data->close_information =
		elm_label_add(window_evas_object_get(data->win));
	elm_label_label_set(data->close_information,
...
static void
frame_close_hide(struct ContactEditViewData *data)
{
	evas_object_del(data->bt1);
	evas_object_del(data->bt2);
...
static void
frame_close_yes_clicked(struct ContactEditViewData *data, Evas_Object * obj,
			void *event_info)
{
	window_destroy(data->win, NULL);
}
...
static void
frame_close_no_clicked(struct ContactEditViewData *data, Evas_Object * obj,
		       void *event_info)
{
...
static void
frame_loading_show(struct ContactEditViewData *data)
{
...
static void
frame_loading_hide(struct ContactEditViewData *data)
{
