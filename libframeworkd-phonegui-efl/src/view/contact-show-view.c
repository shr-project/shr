...
void
contacts_button_call_clicked(struct ContactsWindow *win, Evas_Object * obj,
			     void *event_info)
{
...
void
contacts_button_options_clicked(struct ContactsWindow *win, Evas_Object * obj,
				void *event_info)
{
	Etk_Tree_Row *row = etk_tree_selected_row_get(win->tree);
	if (row != NULL) {
		...
		window_frame_show(win, contacts_options_show,
				  contacts_options_hide);
	}
}

void
contacts_button_save_clicked(struct ContactsWindow *win, Evas_Object * obj,
			     void *event_info)
{
	if (strcmp(etk_entry_text_get(win->entry_name), "")
	    && strcmp(etk_entry_text_get(win->entry_number), "")) {
...
void
contacts_button_back_clicked(struct ContactsWindow *win, Evas_Object * obj,
			     void *event_info)
{
...
void
contacts_button_delete_clicked(struct ContactsWindow *win, Evas_Object * obj,
			       void *event_info)
{
...
void
contacts_button_delete_yes_clicked(struct ContactsWindow *win,
				   Evas_Object * obj, void *event_info)
{
	ogsmd_sim_delete_entry("contacts",
			       g_value_get_int(g_value_array_get_nth
					       (win->tmp_entry, 0)), NULL,
			       NULL);
	pipe_write(pipe_handler, contacts_event, EVENT_MODE_LIST, win);
}

void
contacts_button_delete_no_clicked(struct ContactsWindow *win, Evas_Object * obj,
				  void *event_info)
{
...
void
contacts_button_close_clicked(struct ContactsWindow *win, Evas_Object * obj,
			      void *event_info)
{
	pipe_write(pipe_handler, contacts_event, EVENT_MODE_LIST_CACHED, win);
}
...
contacts_list_show(struct ContactsWindow *win)
{
	...
	evas_object_smart_callback_add(win->bt1, "clicked",
				       contacts_button_new_clicked, win);
	...
	evas_object_smart_callback_add(win->bt2, "clicked",
				       contacts_button_call_clicked, win);
	...
	evas_object_smart_callback_add(win->bt3, "clicked",
				       contacts_button_options_clicked, win);
	...
	etk_tree_col_model_add(win->col1,
			       etk_tree_model_edje_new(UI_FILE, "row"));
	etk_tree_build(ETK_TREE(win->tree));

	Etk_Scrolled_View *scrolled_view =
		etk_tree_scrolled_view_get(ETK_TREE(win->tree));
	etk_scrolled_view_dragable_set(ETK_SCROLLED_VIEW(scrolled_view),
				       ETK_TRUE);
	etk_scrolled_view_drag_damping_set(ETK_SCROLLED_VIEW(scrolled_view),
					   400);
	...
	window_swallow(win, "swallow",
		       etk_embed_object_get(ETK_EMBED(win->container)));
...
contacts_options_show(struct ContactsWindow *win)
{
	...
	window_text_set(win, "name",
			g_value_get_string(g_value_array_get_nth
					   (win->tmp_entry, 1)));
	window_text_set(win, "number",
			g_value_get_string(g_value_array_get_nth
					   (win->tmp_entry, 2)));
	...
	evas_object_smart_callback_add(win->bt1, "clicked",
				       contacts_button_back_clicked, win);
	...
	evas_object_smart_callback_add(win->bt3, "clicked",
				       contacts_button_modify_clicked, win);
	...
	evas_object_smart_callback_add(win->bt4, "clicked",
				       contacts_button_delete_clicked, win);
...
contacts_modify_show(struct ContactsWindow *win)
{
	...
	etk_entry_text_set(win->entry_number,
			   g_value_get_string(g_value_array_get_nth
					      (win->tmp_entry, 2)));
	etk_entry_text_set(win->entry_name,
			   g_value_get_string(g_value_array_get_nth
					      (win->tmp_entry, 1)));
}
...
contacts_new_show(struct ContactsWindow *win)
{
	...
	evas_object_smart_callback_add(win->bt2, "clicked",
				       contacts_button_save_clicked, win);
	window_swallow(win, "button_save", win->bt2);
	evas_object_show(win->bt2);

	win->entry_number = etk_entry_new();
	win->container_number =
		etk_embed_new(evas_object_evas_get
			      (window_evas_object_get(win)));
	etk_container_add(ETK_CONTAINER(win->container_number),
			  win->entry_number);
	etk_widget_show_all(win->container_number);
	window_swallow(win, "number",
		       etk_embed_object_get(ETK_EMBED(win->container_number)));
	...
	window_swallow(win, "name",
		       etk_embed_object_get(ETK_EMBED(win->container_name)));
}
...
contacts_delete_show(struct ContactsWindow *win)
{
	...
	evas_object_smart_callback_add(win->bt1, "clicked",
				       contacts_button_delete_yes_clicked, win);
	...
	evas_object_smart_callback_add(win->bt2, "clicked",
				       contacts_button_delete_no_clicked, win);
...
contacts_sim_full_show(struct ContactsWindow *win)
{
	...
	window_text_set(win, "content",
			D_
			("Your storage is full. Before adding new contacts, you have to delete some old ones."));

	win->bt1 = elm_button_add(window_evas_object_get(win));
	elm_button_label_set(win->bt1, D_("Close"));
	evas_object_smart_callback_add(win->bt1, "clicked",
				       contacts_button_close_clicked, win);
