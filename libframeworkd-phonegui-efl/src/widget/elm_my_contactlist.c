...
_compare_entries(gconstpointer _a, gconstpointer _b)
{
	...
	return strcasecmp(g_value_get_string(g_value_array_get_nth(*a, 1)),
...
_process_entry(gpointer _entry, gpointer data)
{
	...
	g_hash_table_insert(parameters, strdup("id"),
			    GINT_TO_POINTER(g_value_get_int
					    (g_value_array_get_nth(entry, 0))));
	g_hash_table_insert(parameters, strdup("name"),
			    strdup(g_value_get_string
				   (g_value_array_get_nth(entry, 1))));
	g_hash_table_insert(parameters, strdup("number"),
			    strdup(g_value_get_string
				   (g_value_array_get_nth(entry, 2))));
	Etk_Tree_Row *row =
		etk_tree_row_append(ETK_TREE(wd->tree), NULL, wd->col1,
				    parameters, NULL);
...
elm_my_contactlist_add(Evas_Object * parent)
{
	...
	*(wd->widget_pointer) = wd->widget;
	...
	ogsmd_sim_retrieve_phonebook("contacts", _retrieve_callback,
				     wd->widget_pointer);
