...
case "$1" in
    start)
       if [ -f /usr/share/shr-splash/theme/boot.fb ]; then
         ...
         ([ -f /usr/share/shr-splash/theme/boot.fbz ] && zcat /usr/share/shr-splash/theme/boot.fbz > /dev/fb0) &
       fi
    ;;
    stop)
       if [ -f /usr/share/shr-splash/theme/shutdown.fb ]; then
         ...
         ([ -f /usr/share/shr-splash/theme/shutdown.fbz ] && zcat /usr/share/shr-splash/theme/shutdown.fbz > /dev/fb0) &
       fi
    ;;
esac
